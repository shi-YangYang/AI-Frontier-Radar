...
- 长帖在页面中折叠为 "Show more / 显示更多"，抓取时会自动点击展开；链接的缩略显示（如 `…`）会用锚点 href 还原为完整地址。已有入库帖子不会回填，只影响新入库内容。
- 匿名访问只展示部分帖子。需要完整时间线时，在 `/settings → X 数据源 → 打开 X 登录窗口` 登录一次，复用 `.x-browser-public-profile` 持久化 profile。
- 无可用投递目标（飞书 Webhook）时轮询照常执行：帖子入库供 `/posts` 查看，不创建投递事件，账号状态正常；之后配置投递目标只推送新帖，不补发历史。
