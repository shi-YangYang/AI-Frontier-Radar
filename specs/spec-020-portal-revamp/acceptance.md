...
- 搜索：支持按标题/正文关键词查询（后端 `query` 同时匹配 title 与 textContent，管理端搜索同步受益）；空结果正确显示；清空恢复。
- 布局：PC 三列卡片网格（`auto-fill minmax(300px,1fr)`），移动端保持单列列表；内容区在消息页签下加宽至 1000px。
- smoke 新增「用户端消息搜索（标题/正文，空结果）」。
