...
| spec-013 | 微信桥多账号：可绑定多个微信号（各自扫码），账号列表/删除、按账号轮询会话、渠道可选发送账号与目标 |
| spec-014 | 微信推送自动化：绑定即默认接收推送、按账号开关；移除手动投递区块与列表冗余入口 |
| spec-016 | 登录与权限：全站需登录，管理端仅管理员，普通用户绑定端（`/portal`）只管理自己的微信；管理员可在设置页管理用户 |
| spec-017 | 微信按源过滤：绑定端可为每个微信号选择接收哪些监听源（默认全部），轮询按目标配置过滤投递 |
| spec-018 | 源运维与展示：一键删除全部监听源（级联清理）、来源中文友好名与分组展示、每个用户限绑一个微信 |
| spec-019 | 微信会话可靠性：桥缓存并透传 context_token、会话失效快速失败不重试、绑定端会话状态提示 |
| spec-020 | 用户端改版：消息浏览页签 + 微信配置页签；额度进度与会话激活引导；微信推送附加额度 tip |
