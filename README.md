...
| 入口 | 说明 |
| --- | --- |
| `/login` | 登录页；管理员登录后进入管理台，普通用户登录后进入 `/portal`；启用钉钉登录后，登录按钮下方有分割线与「钉钉登录」入口 |
| `/portal` | 普通用户页：只能绑定/解绑自己的微信，查看推送状态 |
| `/settings` -> 用户 | 管理员创建/删除用户、重置密码（首个管理员由 `.env` 种子创建）；钉钉扫码登录在「登录配置」页签配置 |
...
在 `/settings -> 登录配置` 配置钉钉应用凭据并启用后，登录页会在【登录】按钮下方出现分割线与「钉钉登录」按钮（钉钉蓝 + 官方 logo）。

| 项 | 说明 |
| --- | --- |
| 应用类型 | 钉钉开放平台「企业内部应用」即可 |
| 需要填写 | AppKey（client_id）、AppSecret（clientSecret，服务端使用，保存后脱敏显示） |
| 应用权限 | 需在「权限管理 → 通讯录管理」开通**通讯录个人信息读权限**（用于登录后获取 unionId/昵称）；手机号/邮箱权限无需开通 |
| CorpId（可选） | 填写企业 CorpId 后，扫码走「专属账号登录」，仅本企业成员可扫码；留空则任何钉钉账号均可扫码。注意：专属登录要求组织已在钉钉开启「专属账号功能」，未开启时建议留空 |
...
主题包（Source Pack）是「命名 + 描述 + 一组监听源」的管理单元，用户端可以按包一键订阅：

- **管理入口**：`/accounts -> 主题包`。支持新建/编辑包（名称必填且唯一、描述、从现有源多选成员并支持搜索）、排序（上移/下移）、启用/停用与删除。
- **首启种子**：首次启动会确保内置预设源存在，并自动创建首个包「**AI 消息**」（arXiv cs.AI / cs.CL / cs.LG / cs.CV、HF Daily Papers、Techmeme、Hacker News、Reddit r/LocalLLaMA、Product Hunt、OpenAI News、Google AI、Google DeepMind、Anthropic News、AI at Meta、xAI News、AI2 Blog、Moonshot Blog、Mistral、Stability AI、Hugging Face Blog、量子位、GitHub Trending 等约 22 个源）；同名包已存在时跳过，不会重复创建。
- **用户端语义**：`/portal` 接收源支持三段选择 `全部接收 | 按主题包 | 自定义`：
  - 「按主题包」勾选包即自动保存；包内新增的源会**自动纳入推送**，无需用户重新选择；
  - 包卡片显示名称/描述/源数/平台小标；**停用的包置灰不可选**（用户的选择保留，期间收不到该包内容，重新启用后自动恢复）；空包（暂无内容）同样置灰；
  - 「自定义」保留逐源细选（chips 按所属包分组，未分组的源归入「未分组」，chip 前缀平台小标）；取消最后一个源会恢复为「全部接收」（现状语义不变）。
- **投递过滤**：`config.packIds` 存在时按启用包成员的并集投递（解析为空——包全停用/全空包——则**不推送**，而不是回退全部）；否则按 `config.sourceIds` 自定义列表；两者均无则全部接收。
- **删除包**：会级联删除包成员，并清理所有用户选择里对该包的引用（管理端删除确认会显示受影响用户数）；只剩其它包的用户继续按包接收，包全部被删的用户回退为全部接收。

旧的「监听组合」一键添加 API 与面板已由主题包取代（`SOURCE_GROUPS` 配置仅保留作为首启种子数据）。

## 收录规则与删除

- 新添加的源**首轮只锚定最新 1 条**（不入库、不推送），之后只收录比基准更新的内容——不会把历史旧文一次性灌入。
...
- 多名用户可同时扫码绑定；二维码、验证码和取消操作按登录用户隔离，绑定结果只关联该次扫码确认的微信账号。
- 同一微信重新扫码成功后，新连接替换旧连接；旧通道及其待发送任务会停用，已发送历史保留。服务启动时也会清理遗留的重复绑定，仅保留最新连接。
- **额度进度条**：显示本窗口已推送条数（默认 10 条/24 小时，发消息给 ClawBot 后重置）。

### 按源过滤（普通用户）

- 普通用户登录后打开 `/portal`，在「推送设置」卡中选择这个微信号**接收哪些内容**：
  - 三段选择 `全部接收 | 按主题包 | 自定义`（详见「主题包」章节）；每段下方显示当前生效源数；
  - 「按主题包」勾选包即自动保存；「自定义」按包分组的 chips 逐源勾选；
  - 未选择的源/包不会再推送到这个微信号，其他绑定不受影响；保存后立即生效。
...
- 发送前会把相邻行之间的单个换行升级为空行：微信桌面端按 Markdown 渲染（单换行会被折叠），升级后手机与电脑端的换行表现一致。
