...
默认配置已经可以本地启动；`.env` 主要用于覆盖默认值或做首次导入。飞书群自定义机器人 Webhook 可以在启动后通过本地配置页填写，也可以继续通过 `.env` 提供初始值：
...
V1.5 起，轮询间隔、每账号抓取数量、是否排除回复、是否排除转发和飞书 Webhook 可在本地配置页写入 SQLite。运行时优先使用 SQLite 中的 Web 配置；`.env` 继续作为默认值或首次启动种子。SQLite 文件位置由 `SQLITE_PATH` 决定，默认是 `.data/ai-news-monitor.sqlite`。
...
- `/accounts`：监听账号管理，支持账号列表、新增账号、删除账号。
- `/poll-runs`：最近轮询记录，支持时间范围查询、清空查询、分页跳转、删除记录和查看错误详情。
- `/delivery-events`：最近发送记录，支持时间范围查询、清空查询、分页跳转和删除记录。
- `/settings`：配置页，支持保存轮询间隔、每账号抓取数量、是否排除回复、是否排除转发、飞书 Webhook，并可发送飞书测试消息。

配置页全部使用中文文案。飞书 Webhook 保存后不会在页面上显示完整 URL，只显示脱敏预览；测试发送按钮会调用当前已保存的 Webhook，成功和失败都会在页面给出中文反馈。配置页还展示只读运行信息，包括 source mode、SQLite 路径、Redis URL 脱敏预览、服务 host/port/env，以及 X 浏览器或 API 配置摘要；Bearer Token、Cookie 和浏览器 profile 内容不会展示。
