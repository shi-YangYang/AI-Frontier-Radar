...
| `/settings` -> 用户 | 管理员创建/删除用户、重置密码（首个管理员由 `.env` 种子创建）；同页可配置钉钉扫码登录 |
...
在 `/settings -> 用户 -> 钉钉登录` 配置钉钉应用凭据并启用后，登录页会在【登录】按钮下方出现分割线与「钉钉登录」按钮（钉钉蓝 + 官方 logo）。
...
| 账号映射 | 首次钉钉登录自动创建**普通用户**（以钉钉 unionId 唯一对应），管理员可在用户管理中提升角色/删除；同一钉钉账号再次登录映射同一用户 |
