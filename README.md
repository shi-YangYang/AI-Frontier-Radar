...
- `/posts`：消息内容页面，查看已经轮询到的消息内容，支持作者、正文关键词、发布时间范围、检测时间范围、回复/转发筛选。
...
V2.0 起新增 `/posts`“消息内容”页面。页面使用卡片式时间线展示已轮询到的 X 消息，卡片包含作者、正文摘要、北京时间、UTC 时间、检测时间、帖子 ID、回复 / 转发标签、原文链接和飞书投递摘要。点击卡片可打开详情抽屉，查看完整正文、发布时间、检测时间、创建时间、投递事件列表和默认折叠的 raw payload。页面支持按作者、正文、发布时间、检测时间、回复和转发条件查询，支持分页和页码跳转；投递状态只作为卡片摘要和详情信息展示，不支持按投递状态筛选。自动刷新默认开启，间隔 15 秒；发现新内容时会提示点击查看，不强制滚动或打断当前阅读。`/admin/api/posts` 输出中的 `deliveryEvents.lastError` 会在 API 输出层对飞书 webhook 做脱敏处理，不展示完整 webhook。
...
`/`、`/accounts`、`/posts`、`/poll-runs`、`/delivery-events`、`/settings`、`/admin-assets/*` 和 `/admin/api/*` 只允许从 `127.0.0.1`、`::1` 或 `::ffff:127.0.0.1` 访问。如果服务配置为 `HOST=0.0.0.0`，局域网其他机器也会被拒绝访问管理路由和管理静态资源。
